		...
		<Row>
			<Col>
				{container.map((category, categoryIdx) => {
					if (Object.entries(category).length === 0) return <StyledDiv key={categoryIdx} />;

					return (
						<Link key={category.id} to={`${match.url}/${containerIdx}${categoryIdx}`}>
							<StyledDiv>
								<ImgBox>
				...
				render={({ match }) =>
					container.map(
						(category, categoryIdx) =>
							`${containerIdx}${categoryIdx}` === match.params.id && <InfoBox key={category.id} category={category} />
					)
				}
			/>
		</Row>
...
function InfoBox({ category: { thumbnail, temperature, body, title } }) {
	return (
		<Info>
			<div>
				<CategoryImg src={thumbnail} alt="thumbnail" />
			</div>
			<div>
				<div>
					<CategorySpan ice={temperature === 'ice'}>{String(temperature).toUpperCase()}</CategorySpan>
					{title}
				</div>
